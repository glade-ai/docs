...
- **Signing in after firm ownership moves**: When ownership of a firm is transferred to someone who was already on the account as a team member, that person reaches the firm's back office on their next sign-in. Previously a new owner appointed this way was treated as a client and routed to the client dashboard, which showed them signing in as a customer of Glade — sometimes after a first glimpse of the firm, which then disappeared. The firm they own is recognized from the moment the transfer is made, so there is nothing to re-do on the account afterwards.
- **Removing a team member takes effect immediately**: When you revoke someone's access, it stops working at once rather than on a short delay. Previously a removed team member could keep working for up to a minute after the change was made.
- **Resending an invitation**: A team member who has been added but has never signed in can have their invitation email sent again from the Team page, without being deleted and re-added first. Use this when the original invite never arrived or was lost. An invitation that is still valid is sent again as it is; otherwise a fresh one is issued and is good for 7 days. The action is offered only before the person's first sign-in, and disappears once they have signed in.
...
- An invitation can only be resent to someone who has never signed in. A team member who has signed in before and cannot get back in is not an invitation problem — they reach their account through the normal sign-in.
