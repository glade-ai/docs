...
- **Court district**: The report can be narrowed to one or more court districts, so you can answer "which cases are filed in these districts?" rather than only seeing the district on each row. The filter applies to the report on screen and to its CSV export.
  ...
  - **The Court district column and the Court district filter now agree.** The column shows the district assigned to the case, falling back to the district text recorded in case data where no district is assigned. Previously the column read only the case-data text while the filter matched the assignment, so filtering to a district could return rows whose Court district cell was empty — on one firm's Western Pennsylvania report, 89 of 139 rows showed nothing. Cells that were blank start showing a district; no cell that shows a value today goes blank.
  - **District names read consistently.** Where a case has an assigned district, the column prints that district's standard name, so the same court no longer appears under several hand-typed spellings in one report. Cases that fall back to typed text still show it as it was entered.
  - The CSV export and the report's sort order use the same value as the column, so sorting by Court district orders by what you can see.
  - A case still showing only typed text has no district assigned, so it remains outside a district filter. Assigning the district on the case brings it into the filtered report.
...
- The **Court district** filter only returns cases with a district assigned to them. Cases that record a district as free text but have none assigned are excluded from a filtered report, even though the column now shows that text.
