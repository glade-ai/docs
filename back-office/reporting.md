...
- **Court district**: The report can be narrowed to one or more court districts, so you can answer "which cases are filed in these districts?" rather than only seeing the district on each row. The filter applies to the report on screen and to its CSV export.
  ...
  - **Each row is judged on its own district, not on a related case's.** A matter can carry several cases — an associated filing, or a new case created when a chapter converts — and each is assigned its district separately. A case with no district of its own is no longer pulled into a district's report because a sibling case in the same matter has one.
    - This makes the report agree with the case itself. Previously a district-filtered report could list cases whose own summary showed no district at all, which made the report look wrong even to the person who built it.
    - **Expect district-filtered reports to return fewer rows than they used to.** The cases that drop out are cases with no district assigned — they have not been lost, they were never in that district. If a case you expect is missing, assign its district on the case and it returns to the report.
...
- The **Court district** filter only returns cases with a district assigned to them. Cases that record a district as free text but have none assigned are excluded from a filtered report, as are cases where only another case in the same matter carries the district.
- A district assigned to one case in a matter does not carry across to the others. Where every case in a matter should appear in a district's report, each one needs its district assigned.
