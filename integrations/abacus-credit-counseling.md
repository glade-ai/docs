...
- Enrolling a client carries a flat enrollment fee of $25, billed to the firm and shown on the credit counseling pane before the enrollment is sent. This applies when the firm pays for counseling, which is how every firm starts out.
- **A firm can instead have the client pay for the course directly.** Set per provider in the firm's provider settings, this replaces the client's enrollment step with a purchase step: the client buys the course by credit or debit card through Glade's checkout before enrollment is requested, and the enrollment is left off the firm's regular Glade billing. Glade checks that the completed purchase belongs to that client and covers the course being requested before enrolling them. Everything after enrollment — the course link, the certificate, and how it attaches to the case — is identical either way. See [Credit Counseling](../workflows/credit-counseling.md) for what the client sees.
...
| Who pays for the course | Set per provider in the firm's provider settings: the firm pays (the default for every firm), or the client pays for the course themselves before enrollment. |
...
- Client-paid counseling must be paid by credit or debit card through Glade's checkout. It cannot be billed to the firm's invoice or added to a payment plan.
- Whether the client or the firm pays is a firm-wide choice per provider. It cannot be varied case by case, and switching it does not affect clients who have already been enrolled.
