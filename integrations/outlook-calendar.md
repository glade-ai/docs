...
- Real-time notifications from Microsoft are the primary sync path, with the hourly catch-up job and the manual trigger as backstops. Stale or expired delta links are recovered automatically during the next sync run (real-time, manual, or scheduled).
