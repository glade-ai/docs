...
- **Mortgage statement AI extraction**: When a mortgage statement is uploaded to a case, Glade automatically identifies it and extracts the servicer name, loan account number, loan type, origination date, and the servicer's remittance address. The extracted details populate a creditor entry on the case, so the loan information shows up in the creditor list without manual entry. Consecutive monthly statements for the same loan (matched by account number) update the same creditor entry rather than creating duplicates. Extracted values are treated as assistive — any value a team member or client has entered manually for the same field takes precedence over the AI-extracted value.
