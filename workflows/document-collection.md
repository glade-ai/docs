...
- **Identity document extraction**: When a driver's license or passport is uploaded, Glade detects the document type and extracts the filer's legal name and date of birth — and, for driver's licenses, the residential address — into the case record. Other details printed on the document (license or passport number, issue and expiry dates, place of birth) are not captured, since they have no corresponding field on the case record. Extracted values are assistive — a value already entered by a team member or client takes precedence.
...
- Identity document extraction currently populates the primary filer's record. On a joint case, confirm the second filer's name, date of birth, and address manually rather than relying on a co-filer's uploaded license or passport.
