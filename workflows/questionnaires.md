...
- Deduplication of list items is available but requires specifying the field. In a deduplicated list, when you remove an item that has duplicate entries merged into it — for example a creditor on the Master Creditor List that other duplicate rows are linked to — those merged duplicates are removed along with it, so the item stays deleted when the page is refreshed.
- When you remove a row from a list, the removal is saved — including in lists that mirror another list (such as Schedule D Creditors mirroring the Master Creditor List), where removals did not always save before. The row stays removed when the page is refreshed.
- Data imported into a list field — for example from a credit report or the Income Organizer — is saved automatically once the import finishes, the same as values entered by hand.
