...
Alongside the official bankruptcy forms, Glade can generate supplemental forms that a district requires or that support an answer on an official form. Two are newly available:
...
- **Texas Southern Schedule C-1** — the *Supplemental Exemption Analysis* local form filed with the petition in the Southern District of Texas. It is entirely computed: every asset category is carried across from Schedules A/B, C, and D and totalled into gross value, encumbrances, equity, exempt, and non-exempt columns. There is nothing extra to fill in — completing the schedules produces the form. The section appears only on cases filing in the Southern District of Texas.
...
- **Southern District of Ohio** cases can now generate a Chapter 13 plan as well, on the district's Local Form 3015-1. It works the same way: the plan form is available from the calculator for any case that resolves to the district, it is built from the district's own figures, and there is no per-firm setting to switch on. Cases in the district that previously had no plan form available can now produce one. Check the trustee details on the first Southern Ohio plan your firm generates before filing it — the district's figures are newly set up, and the trustee names in particular are worth confirming against your own records.
