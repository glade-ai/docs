...
#### Correcting a recorded amount

If a payment was recorded for the wrong amount — a check entered as $1,500 when it was $1,050, for example — your team can correct the amount on the payment itself rather than deleting and re-recording it. Saving the correction now works; previously it reported a failed save even though the new amount had been stored.

- Only payments recorded outside Glade can have their amount changed, and only when the payment is attached to an invoice. Amounts on payments handled by a payment processor are fixed.
- The corrected amount must be greater than zero and cannot exceed what is still owed on the invoice plus the amount currently recorded on that payment. To record more than the invoice total, edit the invoice instead.
- The invoice updates as soon as the correction is saved. If the new amount covers the invoice in full, the invoice is marked **Paid** and any payment plan on it is completed. If the new amount is lower and the invoice is no longer covered, the invoice reopens to **In Progress** for the remaining balance, and anything waiting on the invoice being paid returns to waiting.
- Notes on a payment can be edited at any time, on any payment, and do not affect the invoice.
...
- The amount on a payment can only be corrected when the payment was recorded outside Glade and is attached to an invoice. Processor-handled payments and outside-of-Glade payments not tied to an invoice cannot have their amount changed.
