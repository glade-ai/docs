...
- A **canceled** booking on a workflow step also stays clickable so it can be rebooked. The card's **View** button opens the scheduler, and its caption reads **"Canceled, select a new time"**; picking a new slot reschedules the booking and returns it to scheduled status. Previously a canceled booking showed a View button that did nothing, leaving the step stuck. Cards for **skipped** bookings remain disabled.
