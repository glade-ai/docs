...
- That still holds when sync has picked up the booking's Outlook or Google event twice. Occasionally the same calendar event reaches Glade as two copies a moment apart; the extra copy is recognized as the booking's own event and ignored. Previously the extra copy was treated as separate busy time — for example Focus Time — and could hide a slot that still had room under the concurrent booking limit, such as the 4th slot on an appointment type that allows 4 overlapping bookings when only 3 were booked. A genuinely separate busy event on the same calendar still takes its place as usual.
